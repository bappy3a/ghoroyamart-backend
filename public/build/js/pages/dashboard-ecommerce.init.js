...
function loadCharts() {
    ...
    if (vectorMapWorldMarkersColors) {
        ...
        var salesLocationMarkers = (window.salesByLocation || []).filter(function (location) {
            return Array.isArray(location.coords) && location.coords.length === 2 && !isNaN(location.coords[0]) && !isNaN(location.coords[1]);
        }).map(function (location) {
            return {
                name: location.name + " (" + location.orders + " orders, ৳" + Number(location.total || 0).toLocaleString("en-US") + ")",
        ...
        worldemapmarkers = new jsVectorMap({
            ...
            selectedMarkers: salesLocationMarkers.length ? [0] : [],
            regionStyle: {
                initial: {
                    stroke: "#9599ad",
                    strokeWidth: 0.25,
    ...
    if (linechartcustomerColors) {
        ...
        var ordersData = (window.revenueChartData && window.revenueChartData.orders) ? window.revenueChartData.orders : [34, 65, 46, 68, 49, 61, 42, 44, 78, 52, 63, 67];
        var earningsData = (window.revenueChartData && window.revenueChartData.earnings) ? window.revenueChartData.earnings : [89.25, 98.58, 68.74, 108.87, 77.54, 84.03, 51.24, 28.57, 92.57, 42.36, 88.51, 36.57];
        var refundsData = (window.revenueChartData && window.revenueChartData.refunds) ? window.revenueChartData.refunds : [8, 12, 7, 17, 21, 11, 5, 9, 7, 29, 12, 35];
        var monthLabels = (window.revenueChartData && window.revenueChartData.months) ? window.revenueChartData.months : ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
